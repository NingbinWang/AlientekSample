...
{			    		   
		...
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1|GPIO_Pin_2;				 // PB1,PB2端口配置
	 	...
	 	GPIO_Init(GPIOB, &GPIO_InitStructure);//B1,B2推挽输出
	 	GPIO_SetBits(GPIOB,GPIO_Pin_1|GPIO_Pin_2);//上拉
		
		
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;				 // F9端口配置
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	 	GPIO_Init(GPIOF, &GPIO_InitStructure);//PF9推挽输出
	 	GPIO_SetBits(GPIOF, GPIO_Pin_9);//上拉
		
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10|GPIO_Pin_8;				 // PF10,PF8端口配置
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //上拉输入
	 	GPIO_Init(GPIOF, &GPIO_InitStructure);//PF10,PF8上拉输入
	 	GPIO_SetBits(GPIOF,GPIO_Pin_10|GPIO_Pin_8);//上拉	
